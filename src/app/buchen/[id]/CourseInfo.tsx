...
export async function CourseInfo({ id, kurs }: { id: string; kurs?: string | null }) {
  ...
  const { data: session } = await supabase
    ...
    .select(
      `
      id,start_date,start_time,city,state,price_cents,
      partner:partners(name,city,state),
      course:courses!inner(id,title,slug,hero_image_url)
    `
    )
    .eq("id", id)
    .maybeSingle();
    ...
    <div className="mb-6 rounded-2xl border border-slate-200 bg-white/80 p-4 shadow-sm">
      <div className="flex items-start justify-between gap-3">
        <div className="space-y-1">
          ...
          <h2 className="text-xl font-semibold text-slate-900">{course?.title ?? "Kurs"}</h2>
        ...
        {course?.hero_image_url && (
          <div className="hidden sm:block relative h-20 w-32 overflow-hidden rounded-xl border border-slate-200 bg-slate-100">
            ...
            <img src={course.hero_image_url} alt={course.title ?? "Kurs"} className="h-full w-full object-cover" />
          </div>
        )}
      </div>
