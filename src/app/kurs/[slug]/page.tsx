...
export default async function CoursePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900">
      ...
      <section className="relative min-h-[250vh] w-screen bg-black z-10">
        <div className="sticky top-0 h-[100svh] w-full overflow-hidden">
          <div className="absolute inset-0">
          {(() => {
            const mediaDesktopRaw = course.slogan_image_url ?? course.slogan_image_mobile_url ?? "";
            const mediaMobileRaw = course.slogan_image_mobile_url ?? course.slogan_image_url ?? "";
            const fallback = "https://naobgnbpvqgutxsaphci.supabase.co/storage/v1/object/public/media/12fd07d0-64ea-40d8-8ef7-a7961b9512fa.webp";

            const renderMedia = (mediaRaw: string, poster?: string) => {
              const media = toUrl(mediaRaw);
              ...
              // Full-bleed background container so video/image covers the entire section.
                ...
                if (vid) {
                  ...
                  return wrap(
                    <iframe
                      ...
                      style={{
                        // Cover both ultra-wide and ultra-tall: start from vw, enforce min on both axes.
                        width: "100vw",
              ...
              return wrap(
                <img
                  src={media ?? fallback}
                  alt="Kurs Impression"
                  className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 min-w-full min-h-full w-auto h-auto object-cover scale-[1.05]"
                />
              );
            };
          ...
          return (
            <>
              <div className="hidden md:block h-full">{renderMedia(mediaDesktopRaw)}</div>
              <div className="block md:hidden h-full">{renderMedia(mediaMobileRaw)}</div>
            </>
          );
        })()}
          </div>
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <Parallax maxY={70}>
              <Reveal delay={80}>
                <div className="mx-auto max-w-5xl px-6 sm:px-10 lg:px-16 space-y-4 text-center">
                  <p className="font-anton text-[70px] sm:text-[80px] lg:text-[90px] leading-[0.95] text-white drop-shadow-[0_12px_30px_rgba(0,0,0,0.55)]">
                    {(course.slogan_line1 || "Music Mission.")}<br />{(course.slogan_line2 || "Lerne von den besten.")}
                  </p>
                  <p className="text-[28px] sm:text-[32px] lg:text-[40px] text-white/85 font-semibold drop-shadow-[0_8px_24px_rgba(0,0,0,0.45)]">
      ...
      <div className="relative z-20 -mt-[60vh]">
        <ConsultBanner overlapOffset="60vh" height="70vh" zIndex="z-30" />
      </div>
